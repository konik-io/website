...
on how Java 6 users can overcome some of the issues with Java 6.

The Konik Library optionally supports Bean Injections, also 
...
which is not JEE5+ or JSR330 compatible.
...
Recommended for invoice validation is this additional dependency. 
If the stack you are using contains any other validation framework such as Hibernate Validator you can use this instead.   
...
Everything is bundled together in one single dll. You can install the Konik library directly via NuGet.
...
You can also browse the https://www.nuget.org/packages/ZUGFeRD.NET/[NuGet Konik package] for details.

=== .NET Example

To help you get started there is also an https://github.com/konik-io/ZUGFeRD.NET-Example[.NET example] available on GitHub. 
