...
on how Java 6 users can overcome this issues.
...
The default option forKonik is to use Java 
...
+JRS330+ which no part of Spring by default see 
...
that do not contain namespace prefix issues. If you still need to 
...
Open Source Bean Validation Provider http://hibernate.org/validator[+Hibernate Validator+] 
...
A Blog on Konik.io deals with the topic of 
http://localhost:8820/blog.html#Dependency%20Injection%20with%20the%20Konik%20library[Konik and dependeny injection]. 
