...
<html lang="<#if (content.language)??>${content.language}<#else>de</#if>">
  <head>
    ...
    <meta name="description" content="<#if (content.meta_description)??>${content.meta_description}<#else>Open Source ZUGFeRD Bibliothek für Java und .NET</#if>"/>
    ...
    <title><#if (content.doctitle)??><#escape x as x?xml>${content.doctitle}</#escape><#else>Open Source Bibliothek für ZUGFeRD konforme Rechnungen.</#if></title>
